...
StripFirstTP = 0; // Remove first time point? Sometimes used at long interval to create buffer between putting channel on scope and starting actual movie
...
for (w = 0; w < subdirs.length; w++) {
	print (base+subdirs[w]);
	current_dir = base+subdirs[w];
	
	if (File.isDirectory(current_dir) && (startsWith(subdirs[w],"1808"))){
		nameList=getFileList(current_dir);
		
		for(i=0;i<nameList.length;i++){
			print (nameList[i]);
//waitForUser(current_dir+nameList[i]);
...
N=0;
while(File.exists(current_dir+"macroLog"+IJ.pad(N,2)+".txt"))	N++;
saveAs("Text", current_dir+"macroLog"+IJ.pad(N,2)+".txt");
...
function runStitch(name) {
	...
	bigfile=current_dir+name+".nd2";
	dir = current_dir+name+File.separator;
	print(bigfile);
